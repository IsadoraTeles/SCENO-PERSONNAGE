...
// #define INOUT_DIFFUSE_BUFFER 13
...
//     int vectorCount = 0;
// 	float SO = 6.0; // sensor offset
// 	float SA = PI * 0.25; // 45 degres
...
//             if (!(x == 0 && y == 0))
//             {
//                 vec2 direction = vec2(x, y) + SO; // distance of sensor

//                 if (dot(normalize(direction), a.direction.xy) > uConfigsAnts.x)
//                 {
...
//         float angle = SA; // define how you calculate the angle here
//         vec3 rotatedVector = rotateAroundUp(vectors[index], angle);
...
//         float angle = 0.9; // define how you calculate the angle here
//         vec3 rotatedVector = rotateAroundUp(wanderDirection, angle);

//         d = mix(d, rotatedVector, mixFactor);
//     }
...
//     int vectorCount = 0;
	// 	float SO = 6.0; // sensor offset
	// 	float SA = PI * 0.25; // 45 degres
	// 	float RA = PI / 12; // rotation angle that we rotate by for turns

	// 	// FRONT sensor
	// 	vec2 Fs = b.position.xy + normalize(b.speed.xy) * SO;
	// 	vec4 fF = texelFetch(sTD2DInputs[IN_TRAILS_BUFFER], ivec2(Fs.xy), 0);
	...
	// 	vec4 fL = texelFetch(sTD2DInputs[IN_TRAILS_BUFFER], ivec2(Ls.xy), 0);
	...
	// 	vec4 fR = texelFetch(sTD2DInputs[IN_TRAILS_BUFFER], ivec2(Ls.xy), 0);
...
//         float angle = SA; // define how you calculate the angle here
//         vec3 rotatedVector = vec3(rotateAroundOrigin(vectors[index].xy, angle), 1.0);
...
//         float angle = SA; // define how you calculate the angle here
//         vec3 rotatedVector = vec3(rotateAroundOrigin(wanderDirection.xy, angle), 1.0);
