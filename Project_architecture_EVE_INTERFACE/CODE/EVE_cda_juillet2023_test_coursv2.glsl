...
Agent ReadAgent(ivec2 coord)
{
	...
	vec4 color = texelFetch(sTD2DInputs[IN_COLOR_BUFFER], coord, 0);

	a.color.rgb = color.rgb;
...
{
	vec3 posRandom = texelFetch(sTD2DInputs[IN_INIT_POS], posOnTexture, 0).rgb; // already mapped between 0 and res
	...
	Agent a;
	a.position.x = posRandom.r * (uRes.x - 1.0);
	a.position.y = posRandom.g * (uRes.x - 1.0);
	a.position.z = posRandom.b * (uRes.y - 1.0);
	a.speed = velRandom;
	...
	{
		a.espece = 1;
		a.color = vec4(1.0, 0.0, 0.0, a.health);
	}
	else if (a.DNA.a <= 0.7)
	{
		a.espece = 2;
		a.color = vec4(0.7, 0.0, 1.0, a.health);
	}
	else if (a.DNA.a <= 1)
	{
		a.espece = 3;
		a.color = vec4(0.4, 1.0, 1.0, a.health);
...
Agent crossoverAndMutate(ivec2 posOnTexture, Agent a1, Agent a2) 
{
	...
	for(int i = 0; i < 4; i++) 
	{
		...
		if(midpoint > 3) // Assuming midpoint is the determining factor for inheriting properties
		{
			...
			child.color.rgb = a1.color.rgb;
			child.maxSpeed = a1.maxSpeed;
		}
		else
		{
			child.position = a2.position;
			child.color.rgb = a2.color.rgb;
...
vec3 calculateForce(inout Agent a, vec3 targetPos, float targetMass, float targetSpecies, float targetColor, float mapFactor) 
{
    ...
    if(targetSpecies == 1.0) 
    { 
        // BOIDS
		strength *= -2.8;  // no changes
		a.health -= 0.0001;
    } 
	else if(targetSpecies == 0.7) 
    { 
        // ANTS 
		strength *= -1.9;  // no changes
		a.health -= 0.0001;
    } 
...
vec3 AntPhysarumBehavior(inout Agent a, float mixFactor, float forceFactor)
{
...
void UpdateAgent(inout Agent agent) 
{
	...
	agent.health -= 0.001 * agent.size * 0.01;
...
void Write (Agent a, Food foods[7], ivec2 coord, int closest)
{
    ...
    if (withinBoundaries && a.health > 0) 
    {
		imageStore(mTDComputeOutputs[IN_POSITION_AND_MASS_BUFFER], a.myCoord, TDOutputSwizzle(vec4(a.position, a.mass)));
		imageStore(mTDComputeOutputs[IN_VELOCITY_AND_SIZE_BUFFER], a.myCoord, TDOutputSwizzle(vec4(a.speed, a.size)));
		imageStore(mTDComputeOutputs[IN_DIRECTION_AND_HEALTH_BUFFER], a.myCoord, TDOutputSwizzle(vec4(a.direction, a.health)));
		imageStore(mTDComputeOutputs[IN_DNA_BUFFER], a.myCoord, TDOutputSwizzle(a.DNA));
		...
		imageStore(mTDComputeOutputs[IN_COLOR_BUFFER], a.myCoord, TDOutputSwizzle(vec4(a.color.r, a.color.g, a.color.b, a.color.a)));
    }
	...
	else
	{
		...
		imageStore(mTDComputeOutputs[IN_COLOR_BUFFER], coord, TDOutputSwizzle(vec4(0)));
...
void main()
{
	...
	if (ActiveAgent()) // if agent exists
	{
		if (AliveAgent(agent))
		{
			...
			vec3 foodTarget;
			float useRememberedProb = 0.6; // 50% chance to use remembered food location
			...
			{
				// Seek food target
				SeekTarget(agent, foodTarget, uSteerConfigs.z * agent.DNA.r * 3, uSeekSettings.z);
			...
			if(agent.espece == 1 ) // BOIDS
			{
				...
				vec3 chasingForce = ChasePrey(agent, 0.7)* DNA1scaled;

				ApplyForce(agent, alignForce + cohesionForce + separationForce + chasingForce + avoidPredatorForce);
			...
			else if(agent.espece == 2) // PHYSARUM
			{
				if(uDNA == 1)
				{
					...
					DNA1scaled = Map(agent.DNA.g, 0, 1, 0.0, 1);
				...
				ApplyForce(agent, antsTurns * DNA0scaled + chasingForce);				
			}
