...
// vec3 GravitationalAttractionAverage(inout Agent a) // avarage of the forces around
// {   
//     Agent b = a;
...
//     if (totalAgents > 0) 
//     {
//         totalForce /= totalAgents;
// 		totalForce = normx(totalForce);
// 		totalForce *= uSteerConfigs.y;
// 		vec3 steer = totalForce - b.speed;

// 		return steer;
//     }
// }

// vec3 GravitationalAttractionBiggest(inout Agent a) // biggest gravitational force applied
// {   
//     Agent b = a;
...
//                         vec3 steer = calculateForce(b, targetPos, targetMass, targetSpecies, targetColor, mapFactor);
//                         float forceMagnitude = length(steer); // get the magnitude of the force
...
//     // If biggestForce is still zero vector, it means there are no forces acting on the agent, thus initiate wandering behavior
//     if (maxForceMagnitude == 0.0)
//     {
//         vec3 d = vec3(random2(sin(uTime * 0.2)), random2(cos(uTime * 5.2)), random2(sin(uTime * 14.2)));
//         d = map_vector(d, -2, 2);
//         return normalize(d) * 4;
//     }
//     else
//     {
//         biggestForce = normx(biggestForce);
//         biggestForce *= uSteerConfigs.y;
//         vec3 steer = biggestForce - b.speed;
		...
		for(int j = 0; j < uRangeCroud; j++)
		{
			...
			else
			{
				...
				else
				{
					...
					else if(a.espece == 1 && other.espece == 3)
					{
						// Predator gains health, prey loses health
						a.health += 0.001;
						other.health -= 0.001;
					}
					else if(a.espece == 2 && other.espece == 3)
					{
						// Predator gains health, prey loses health
						a.health += 0.01;
						other.health -= 0.001;
...
//     int i = 0;
// 	float uBoidAvoidanceFactor = uAvoidBoidsFactor;
...
// vec3 NeighborhoodTurns1(Agent a)
// {
...
//                     {
//                         maxTrail = level.r;
//                         maxTrailVector = normalize(vec3(x, y, 0));
//                     }
//                 }
//             }
//         }
...
//     // If the ant finds a trail, it will follow the trail.
//     if (maxTrail >= 0.01)
//     {
//         vec3 desired = maxTrailVector * a.maxSpeed;
//         vec3 steer = desired - a.speed;
//         d = steer;

//         // Limit the maximum steering force.
//         float maxSteer = 0.01; // You can adjust this value.
//         if (length(d) > maxSteer)
//         {
//             d = normalize(d) * maxSteer;
//         }
//     }
//     // If there is no trail, the ant will choose a random direction.
//     else
//     {
//         d = vec3(random2(sin(uTime * 0.2)), random2(cos(uTime * 5.2)), random2(sin(uTime * 14.2)));
//         d = map_vector(d, -1, 1);
...
// vec3 NeighborhoodTurns2(Agent a)
// {
//     vec3 vectors[100];
//     vec3 maxTrailVector = vec3(0);
//     float maxTrail = uConfigsAnts.y;
...
//     // Loop through each neighborhood cell in range
//     for (int x = -uRangeNeighbors.z; x <= uRangeNeighbors.z; x++)
//     {
//         for (int y = -uRangeNeighbors.z; y <= uRangeNeighbors.z; y++)
//         {
...
//     else
//     {
//         // Ant is stuck, let it wander around to explore the area
...
//         d = mix(d, wanderDirection, 0.5); // Mix the current direction with a random direction
//     }
...
// vec3 NeighborhoodTurns3(Agent a)
// {
//     vec3 maxTrailVector = vec3(0);
//     float maxTrail = uConfigsAnts.y;
// 	float uBoidAvoidanceFactor = uAvoidBoidsFactor;
...
vec3 AntPhysarumBehavior(Agent a)
{
...
void UpdateAgent(inout Agent agent) 
{
	...
	vec3 noise = vec3(random2(agent.position.x), random2(agent.position.y), random2(agent.position.z)) * 2.0 - 1.0; // Assume random2() returns a random number between -1 and 1.
	...
	agent.health -= random2(agent.size)* 0.001;
	...
	if (ActiveAgent()) // if agent exists
	{
		if (AliveAgent(agent))
		{
			...
			if (length(foodTarget) > 0) 
			{
				...
				if(distance(agent.position, foodTarget) < 2)
				{
					agent.health += 0.001;
				}
			}
			...
			else if(agent.espece == 2) // PHYSARUM
			{
				if(uDNA == 1)
				{
					...
					if(DNA2scaled < 1.0)
					{
						ScapeCroudDirection(agent, DNA2scaled);
					}

				}

				vec3 trailForce = AntPhysarumBehavior(agent);
				ApplyForce(agent, trailForce * DNA0scaled);
			...
			else if (agent.espece == 3) // PP
			{
				...
				ApplyForce(agent, gravForce);
				
				if(agent.crouded >= 4)
				{
					ScapeCroudDirection(agent, agent.crouded * 0.1);
				}			
			}
			...
			if (closest > -1 && agent.health < 5)
			{
				...
				if(distance(agent.position, foods[closest].position) < 2)
				{
					agent.health += 0.001;
				}
			}
