...
Agent ResetAgents(ivec2 posOnTexture)
{
	...
	else if (a.DNA.a <= 1)
	{
		...
		if(colorRandom < 0.4)
		{
			...
			a.orientation = random2(Index()* 0.01) * TWOPI; // random angles from 0 to 2PI and multiply by 3.14*2 to get full circle
		}
		else if(colorRandom < 0.6)
		{
			a.color = vec4(0.4, 1.0, 0.5, a.health);
			a.orientation = random2(Index() * 0.01) * TWOPI;
		}
		else 
		{
...
Agent crossoverAndMutate(ivec2 posOnTexture, Agent Myself, ivec2 mateCoord) 
{
	...
	//child.DNA.a = 0.7;
		...
		{
			child.position = Myself.position;
			//child.color = a1.color;
			child.maxSpeed = Myself.maxSpeed;
		}
		else
		{
			child.position = mate.position;
...
// Agent crossoverAndMutate(ivec2 posOnTexture, Agent Myself, ivec2 myMateCoord) 
// {
// 	Agent child;
// 	child = ResetAgents(posOnTexture);
// 	Agent mate = ReadAgent(myMateCoord);
...
// 	if (0 > midpoint) 
// 	{
// 		child.DNA.r = Myself.DNA.r;
// 	} 
// 	else 
// 	{
// 		child.DNA.r = mate.DNA.r;
// 	}

// 	// for i=1
// 	if (1 > midpoint) 
// 	{
// 		child.DNA.g = Myself.DNA.g;
// 	} 
// 	else 
// 	{
// 		child.DNA.g = mate.DNA.g;
// 	}

// 	// for i=2
// 	if (2 > midpoint) 
// 	{
// 		child.DNA.b = Myself.DNA.b;
// 	} 
// 	else 
// 	{
// 		child.DNA.b = mate.DNA.b;
// 	}

// 	// for i=3
// 	if (3 > midpoint) 
// 	{
// 		child.DNA.a = Myself.DNA.a;
// 	} 
// 	else 
// 	{
// 		child.DNA.a = mate.DNA.a;
// 	}
...
// 			else if(i == 1) 
// 			{
// 				child.DNA.g = clamp(random2(posOnTexture.x * uTime + i * 0.01), 0.0, 1.0);
// 			}
// 			else if(i == 2) 
// 			{
// 				child.DNA.b = clamp(random2(posOnTexture.x * uTime + i * 0.01), 0.0, 1.0);
...
// 			else if(i == 3) 
// 			{
// 				child.DNA.a = clamp(random2(posOnTexture.x * uTime + i * 0.01), 0.0, 1.0);
// 			}
// 		}
// 	}
...
// 	//Crossover
// 	if (random2(posOnTexture.x * uTime) > 0.5)
// 	{
...
// 	if (random2(posOnTexture.y * uTime) > 0.5)
// 	{
...
// 	// Assigning these properties outside the loop to avoid constant overwriting
// 	if(random2(posOnTexture.x * uTime) > 0.5) // Assuming midpoint is the determining factor for inheriting properties
// 	{
...
// 	{
// 		child.espece = 3;
// 		child.color = vec4(0.4, a2.color.g, a1.color.b, child.health); 
// 	}
...
// vec3 AntPhysarumBehavior(Agent a, float mixFactor, float forceFactor) // MAGENTA CHECKING
// {
//     vec3 boidAvoidanceForce = vec3(AvoidBoids(a));
...
//                     // changed condition to only consider magenta level
//                     if (level.b > maxTrail)
//                     {
//                         maxTrail = level.b;
//                         vectorCount = 0;
...
void UpdateAgent(inout Agent agent) 
{
	if(AliveAgent(agent))
	{
		...
		agent.speed += noise * 0.01; // Tune the 0.1 factor to increase or decrease the noise intensity
...
// void WriteDiffuseTexture(Agent a, ivec2 posOnBuffer) // MAGENTA WRITING
// {
//     float uTrailReinforcementFactor = 0.01;
...
//             // average it only if neighbor is species 2 (magenta)
//             if (especeNeighbor == 0.7)
//             {
//                 avg += texelFetch(sTD2DInputs[IN_TRAILS_BUFFER], coord, 0).b;
//             }
//         }
//     }
...
//     // Reinforce existing trails if current agent is species 2 (magenta)
//     if (a.color.r == 0.7)
//     {
//         avg += uTrailReinforcementFactor; // Increase the trail level by a small amount at each time step
//     }
...
void main()
{
	...
	if (ActiveAgent()) // if agent exists
	{
		...
		if (AliveAgent(agent))
		{
			...
			else if(agent.espece == 2) // PHYSARUM
			{
				if(uDNA == 1)
				{
					...
					DNA2scaled = Map(agent.DNA.b, 0, 1, 1.0, 5);
				}

				vec3 antsTurns = AntPhysarumBehavior(agent, DNA1scaled, DNA0scaled);
				vec3 avoidPredatorForce = AvoidPredator(agent, 0.4) * DNA2scaled - 0.5;
			...
			else if (agent.espece == 3) // PP
			{
				...
				vec3 chasingForce = ChasePrey(agent, 0.1) * DNA0scaled;
			...
			// 				vec4 neighborInfo = texelFetch(sTD2DInputs[INOUT_DRAW_MASS_COORD_BUFFER], ivec2(round(neighborPos)), 0);
			// 				myMateCoord = ivec2(neighborInfo.gb);
			// 				float neighborColor = neighborInfo.a;

			// 				if(neighborColor > 0 && neighborColor == agent.color.r) //reproduction intra species
			// 				{
			// 					float neighborHealth = texelFetch(sTD2DInputs[INOUT_DIRECTION_AND_HEALTH_BUFFER], ivec2(myMateCoord), 0).a;
		...
		{
			if(validMateFound) // Only proceed with reproduction if a valid mate has been found
			{
				float born = random2((sin(uTime* .849)));

				if(born < uReproductionChance)
				{
					Agent child = crossoverAndMutate(coord, agent, mateCoord); // like reset agent
					agent = child;
					UpdateAgent(agent);
				}
				else
