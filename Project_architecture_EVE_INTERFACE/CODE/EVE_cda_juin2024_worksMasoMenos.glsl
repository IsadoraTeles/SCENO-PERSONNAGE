...
vec3 AvoidPredator(inout Agent agent, float avoiding) 
{
	...
	float maxSteerForce = Map(bagent.DNA.r + bagent.DNA.b, 0, 2, 0.1, uSteerConfigs.z + 0.1);
...
vec3 ChasePrey(inout Agent agent, float chasing) 
{
	...
	float maxSteerForce = Map(bagent.DNA.r + bagent.DNA.b, 0, 2, 0.5, uSteerConfigs.z + 0.5);
	...
	if (ActiveAgent(coord)) // if agent exists
	{
		...
		if (AliveAgent(agent))
		{
            ...
            if (agent.health < 5)
            {
                ...
                {
					float maxSteerForce = Map(agent.DNA.r + agent.DNA.b, 0, 2, 0.5, 4);
					float seekfood = Map(agent.DNA.r + agent.DNA.g, 0, 2, 2, 20);
			...
			else if (agent.espece == 3) // PP
			{
				if(uDNA == 1)
				{
					...
					DNA2scaled = Map(agent.DNA.b, 0, 1, -30, 30);
					DNA3scaled = Map(agent.DNA.g, 0, 1, -30, 30);
				}
				...
				ApplyOrientationForcePPSAgent(agent, radians(uAlphaPPS) + DNA2scaled, radians(uBetaPPS) + DNA3scaled);
			}
