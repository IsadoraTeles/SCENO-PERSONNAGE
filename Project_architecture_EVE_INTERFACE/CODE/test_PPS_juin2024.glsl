...
void UpdateAgent(inout Agent agent)
{
    if (AliveAgent(agent))
    {
        ...
        if (uNoiseOn == 1.0)
        {
            vec3 noise = vec3(random2(uTime * 0.1975 * Index(agent.myCoord)), 
                              random2(uTime * 0.75 * Index(agent.myCoord)), 
                              random2(Index(agent.myCoord) * uTime * 0.15)) * 2.0 - 1.0;
            agent.speed += noise * 0.05;
        }
...
void Write(Agent a, Food foods[13], ivec2 coord, int closest)
{
    ...
    for (int i = 0; i < uCounts.y; i++)
    {
        ...
        foods[i].foodColor = foodInfo;
        foods[i].index = i;
        imageStore(mTDComputeOutputs[INOUT_DRAW_FOOD_BUFFER], ivec2(round(foods[i].position.xy)), TDOutputSwizzle(foodInfo));
        imageStore(mTDComputeOutputs[INOUT_DRAW_SPEED_HEALTH_BUFFER], ivec2(round(foods[i].position.xy)), TDOutputSwizzle(foodInfo));
