...
Agent ResetAgents(ivec2 posOnTexture)
{
	...
	a.direction = random_direction * 2.0 - 1.0;
		...
		for(int y = -range; y <= range; y++)
		{
			for(int z = -range; z <= range; z++)
			{
				if (!(x == 0 && y == 0 && z == 0)) // NOT CURRENT CELL
				{
					...
					if (targetSpecies == agent.espece) 
					{
						float d = distance(agent.position, coord);

						if (d < 5) // perception radius
						{
							steer += coord;
							count++;
						}
...
vec3 AvoidPredator(Agent agent) 
{
    vec3 steer = vec3(0.0, 0.0, 0.0);
    int count = 0;
	float angle = 0.1;
	...
	if (ActiveAgent()) // if agent exists
	{
		if (AliveAgent(agent))
		{
			...
			else if (agent.espece == 3) // PP
			{
				...
				vec3 gravForce = GravitationalAttractionHybrid(agent) * DNA1scaled;
			
				if(agent.crouded >= 4)
				{
					ScapeCroudDirection(agent, agent.crouded * DNA0scaled);
				}

				ApplyForce(agent, gravForce * DNA1scaled);
