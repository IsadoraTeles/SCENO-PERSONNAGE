...
Agent ReadAgent(ivec2 coord)
{
	...
	{
		a.espece = 3;
		a.color = vec4(0.4, 1, color.b, a.health);
	}
	return a;
}
...
Agent ResetAgents(ivec2 posOnTexture)
{
	...
	// vec3 posRandomX = vec3(random2(posOnTexture.x * uTime * 0.5), 0, 0);
	// vec3 posRandomY = vec3(0, random2(posOnTexture.y * uTime * 0.01), 0);
	// vec3 posRandom = posRandomX + posRandomY;
...
// 		{
// 			float mutation = random2(posOnTexture.y * uTime * 0.1129) * 2 - 1; // range [-1,1]
// 			child.DNA[i] += mutation * 0.8; // adjust the mutation factor as needed
// 			child.DNA[i] = clamp(child.DNA[i], 0.0, 1.0); // make sure DNA stays in the [0,1] range
// 		}
...
// 	// 	float colorRandom = random2(Index() * uTime * 0.1975);

// 	// 	if(colorRandom < 0.4)
// 	// 	{
// 	// 		child.color = vec4(0.4, 1, 0.2, child.health);
// 	// 	}
// 	// 	else if(colorRandom < 0.6)
// 	// 	{
// 	// 		child.color = vec4(0.4, 1, 0.5, child.health);
// 	// 	}
// 	// 	else 
// 	// 	{
// 	// 		child.color = vec4(0.4, 1, 0.8, child.health); 
// 	// 	}
// 	// }
...
void UpdateAgent(inout Agent agent) 
{
	...
	agent.health -= random2(agent.size * uTime * 0.191)* 0.001;
...
void Write (Agent a, Food foods[7], ivec2 coord, int closest)
{
    ...
    if (withinBoundaries && a.health > 0) 
    {
		...
		imageStore(mTDComputeOutputs[IN_COLOR_BUFFER], coord, TDOutputSwizzle(a.color));
