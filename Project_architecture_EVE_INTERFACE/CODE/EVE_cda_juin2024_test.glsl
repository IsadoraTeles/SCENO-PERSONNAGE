...
void ApplyOrientationForcePPSAgent(inout Agent agent, float alpha, float beta) {
    ...
    agent.orientation = mod(agent.orientation, TWOPI);
...
{
    if (AliveAgent(agent)) {
        agent.force = vec3(0.0); // Reset the force at the beginning of the update
        if (agent.espece == 3) {
            vec3 direction = vec3(cos(agent.orientation), sin(agent.orientation), 0.0);
...
void Write(Agent a, Food foods[13], ivec2 coord, int closest)
{
    ...
    if (AliveAgent(a)) 
    {
        ...
        if (a.espece == 3)
        {
            ...
            imageStore(mTDComputeOutputs[OUT_ORIENTATIONS_BUFFER], coord, TDOutputSwizzle(vec4(normalizedN, a.DNA.g, normalizedForce, a.orientation)));
        }
        else
        {
    ...
    if (ActiveAgent(coord)) // if agent exists
    {
        ...
        if (AliveAgent(agent))
        {
            ...
            else if (agent.espece == 3) // PPS
            {
                if(uDNA == 1)
                {
                    DNA0scaled = Map(agent.DNA.r, 0, 1, 0.5, 3);
                    DNA1scaled = Map(agent.DNA.g, 0, 1, 0.5, 3);
                    DNA2scaled = Map(agent.DNA.b, 0, 1, -10, 10);
                    DNA3scaled = Map(agent.DNA.g, 0, 1, -10, 10);
                }
                ...
                ApplyOrientationForcePPSAgent(agent, radians(uAlphaPPS + DNA3scaled), radians(uBetaPPS + DNA3scaled));
            }
