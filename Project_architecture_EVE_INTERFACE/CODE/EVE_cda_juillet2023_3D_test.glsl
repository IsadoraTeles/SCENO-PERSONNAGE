...
struct Agent
{
	...
	float elevation; // Z ANGLE FOR EACH AGENT
...
{
    return fract(sin(dot(st.xyz, vec3(12.9898, 78.233, 34.765))) * 43758.5453123);
}
...
float randomRange(vec3 st, float min, float max) 
{
    return mix(min, max, fract(sin(dot(st.xyz, vec3(12.9898, 78.233, 34.765))) * 43758.5453123));
}

vec4 randomVec4Range(ivec3 posOnTexture, float uTime, float min, float max) 
{
    return vec4(
        randomRange(vec3(posOnTexture) + vec3(sin(uTime * .49)), min, max),
        randomRange(vec3(posOnTexture) + vec3(cos(uTime * .899)), min, max),
        randomRange(vec3(posOnTexture) * vec3(sin(uTime * .0979)), min, max),
        randomRange(vec3(posOnTexture) + vec3(sin(uTime * 0.265)), min, max)
    );
}

vec3 randomVec3Range(ivec3 posOnTexture, float uTime, float min, float max) 
{
    return vec3(
        randomRange(vec3(posOnTexture) + vec3(sin(uTime * .49)), min, max),
        randomRange(vec3(posOnTexture) + vec3(cos(uTime * .029)), min, max),
        randomRange(vec3(posOnTexture) + vec3(sin(uTime * .2279)), min, max)
    );
}

float gaussianRand(vec3 seed) 
{
    // Box-Muller transformation
    float u = random(seed);
    float v = random(seed + vec3(1234.5678, 213.458, 87.912)); // arbitrary numbers to change the seed
...
int Index (ivec3 coord)
{
	...
	ivec3 xyz = ivec3(uRes.x, uRes.y, uRes.z); // uResolution
	int index = (coord.x + xyz.x * (coord.y + xyz.y * coord.z));
	return index;
}

bool ActiveAgent (ivec3 coord)
{
...
Agent ReadAgent(ivec3 coord)
{
	Agent a;
	vec4 dirHealth = texelFetch(sTD3DInputs[INOUT_DIRECTION_AND_HEALTH_BUFFER], coord, 0);
	a.health = dirHealth.a;
	
	vec4 posMass  = texelFetch(sTD3DInputs[INOUT_POSITION_AND_MASS_BUFFER], coord, 0);
	vec4 velSize = texelFetch(sTD3DInputs[INOUT_VELOCITY_AND_SIZE_BUFFER],coord, 0);
	vec4 dna = texelFetch(sTD3DInputs[INOUT_DNA_BUFFER], coord, 0);
	ivec3 agentCoord = coord;
	
	vec4 foodMemoryCOLORB_IN = texelFetch(sTD3DInputs[INOUT_MEMORYFOOD_COLOR_B_BUFFER], coord, 0);
	...
	{
		a.foodMemory = vec3(uRes.x * 0.5, uRes.y * 0.5, uRes.z * 0.5);
	}

	vec2 orientationElevationIN = texelFetch(sTD3DInputs[IN_ORIENTATIONS_BUFFER], coord, 0).ba;
	a.orientation = orientationElevationIN.y;
	a.elevation = orientationElevationIN.x;
	...
	else if (a.DNA.a <= 1.0)
	{
		a.espece = 3;
		a.color.r = 0.4;
		a.color.g = texelFetch(sTD3DInputs[INOUT_COLOR_AGENTS_BUFFER], ivec3(a.position.xyz), 0).g;
		a.color.a = a.health;
...
Agent ResetAgents(ivec3 posOnTexture)
{
	vec3 posRandom = vec3(random2(posOnTexture.x * fract(uTime) * 0.05) * uRes.x, random2(posOnTexture.y * fract(uTime) * 0.001) * uRes.y, random2(posOnTexture.z * fract(uTime) * 0.0015) * uRes.z);
	...
	vec4 dna = vec4(random2(posOnTexture.x * fract(uTime) * 20.905), random2(posOnTexture.y * fract(uTime) * 9.001), random2(posOnTexture.z * fract(uTime) * 0.715), random2(posOnTexture.x * fract(uTime) * 9.905));

	Agent a;
	a.position = posscaledtoresolution;
	a.speed = velRandom;

	vec3 random_direction = vec3(random2(posOnTexture.x + uTime * 0.4), random2(posOnTexture.y + 0.7 + uTime * 0.915), random2(posOnTexture.z + uTime * 0.315));
	...
	else if (a.DNA.a <= 1)
	{
		...
		if(colorRandom < 0.4)
		{
			a.color = vec4(0.4, 0.1, 0.0 , a.health);
			...
			a.elevation = random2(Index(posOnTexture.zxy)* 0.01) * TWOPI; // random angles from 0 to 2PI and multiply by 3.14*2 to get full circle
		}
		else if(colorRandom < 0.6)
		{
			a.color = vec4(0.4,  0.5, 0.0, a.health);
			a.orientation = random2(Index(posOnTexture) * 0.01) * TWOPI;
			a.elevation = random2(Index(posOnTexture.zxy)* 0.01) * TWOPI; // random angles from 0 to 2PI and multiply by 3.14*2 to get full circle
		}
		else 
		{
			a.color = vec4(0.4, 0.9, 0.0, a.health);
			a.orientation = random2(Index(posOnTexture)* 0.01) * TWOPI;
			a.elevation = random2(Index(posOnTexture.zxy)* 0.01) * TWOPI; // random angles from 0 to 2PI and multiply by 3.14*2 to get full circle
		}
	}
...
int getRandomInt(vec3 seed) 
{
    // Create two pseudo-random numbers from the sine of the dot product
    // The randomness of the number is based on the seed value
    float rand1 = fract(sin(dot(seed.xy, vec2(12.9898, 78.233))) * 43758.5453);
    float rand2 = fract(sin(seed.z * 12.9898) * 43758.5453);

    // Scale the random numbers to be between 0 and 6
    int result1 = int(rand1 * 7.0);
    int result2 = int(rand2 * 7.0);
    
    // Combine the two results in some way
    // In this example, we just add them together, but you might want to
    // use a different method depending on your use case
    int result = result1 + result2;
    
    return result;
}

Agent crossoverAndMutate(ivec3 posOnTexture, Agent Myself, ivec3 mateCoord) 
{
	Agent child;
	...
	child.health = 5 * Map(random2(child.size + child.position.x * child.position.y * uTime * 0.191), 0.0, 1.0, 2.0,  100 * child.DNA.r);
...
void Attractors(inout Agent a)
{
	...
	{
		vec4 attractor = texelFetch(sTD3DInputs[IN_ATTRACTORS_BUFFER], ivec3(i, 0), 0);
		vec3 attractorPos = attractor.rgb;
		
		float dist = distance(attractorPos.xy, b.position.xy);

		if (dist < uSeekSettings.x)
		{
			a.health -= 0.5;
		}
		else
        ...
        for(int y = -uRangeNeighbors.x; y <= uRangeNeighbors.x; y++)
        {
            for(int z = -uRangeNeighbors.x; z <= uRangeNeighbors.x; z++)
            {
                if (!(x == 0 && y == 0 && z == 0)) // NOT CURRENT CELL
                {
                    ...
                    vec3 targetSpeciesAndColor = texelFetch(sTD3DInputs[INOUT_COLOR_AGENTS_BUFFER], ivec3(targetPos), 0).rba;
                    ...
                    if(targetHealth > 0) 
                    {
                        vec4 targetMassCoord = texelFetch(sTD3DInputs[INOUT_DRAW_MASS_COORD_BUFFER], ivec3(targetPos), 0);
...
// bool isOnRightSide(vec2 p, vec2 v, vec2 op) // position, speed and other position IN 2D
// {
//     vec2 b = p + v;
//     return ((b.x - p.x) * (op.y - p.y) - (b.y - p.y) * (op.x - p.x)) > 0.0;
// }

bool isOnRightSide(vec3 p, vec3 v, vec3 op) // position, speed (used as a direction vector here) and other position
{
    vec3 normal = normalize(v);
    vec3 diff = op - p;
    return dot(normal, diff) > 0.0;
}

vec3 updateLandR(Agent b)
{
    float left = 0.0;
    float right = 0.0;
    ...
    for (int x = -uRadiusPPS; x <= uRadiusPPS; x++)
    {
        ...
        {
            for(int z = -uRadiusPPS; z <= uRadiusPPS; z++)
            {
                if (!(x == 0 && y == 0 && z == 0)) // NOT CURRENT CELL
                {
                    vec3 targetPos = vec3(b.position.x + x, b.position.y + y, b.position.z + z);
                    ...
                    {
                        if(isOnRightSide(b.position, b.speed, targetPos))
                        {
                            right += 1.0;
                        }
                        else
...
void ApplyOrientationForcePPSAgent(inout Agent agent, float magnitude)
{
    ...
    vec3 direction = vec3(cos(agent.orientation) * sin(agent.elevation), sin(agent.orientation) * sin(agent.elevation), cos(agent.elevation));
    ...
    // UPDATE agent orientation and elevation
    agent.orientation += delta_phi * dampingFactor;  // apply the damping factor
    agent.elevation += delta_phi * dampingFactor;  // adjust elevation as per your use case

    direction = vec3(cos(agent.orientation) * sin(agent.elevation), sin(agent.orientation) * sin(agent.elevation), cos(agent.elevation));
...
vec3 AntPhysarumBehavior(Agent a, float mixFactor, float forceFactor)
{
    ...
    for (int x = -uRangeNeighbors.z; x <= uRangeNeighbors.z; x++)
    {
        ...
        {
            for (int z = -uRangeNeighbors.z; z <= uRangeNeighbors.z; z++)
            {
                if (!(x == 0 && y == 0 && z == 0)) // NOT CURRENT CELL
                {
                    vec3 direction = vec3(x, y, z);

                    if (dot(normalize(direction), a.direction) > uConfigsAnts.x) // sensing angle
                    {
                        ivec3 coord = ivec3(round(a.position + direction));

                        vec3 level = texelFetch(sTD3DInputs[IN_TRAILS_BUFFER], ivec3(coord), 0).rgb;

                        if (level.r == maxTrail && level.g == maxTrail && level.b == maxTrail)
                        {
                            vectors[vectorCount] = normalize(vec3(x, y, z));
                            vectorCount++;
                        }
    ...
    vec3 currentDirection = normalize(a.speed);
    ...
    else
    {
        ...
        d = mix(d, wanderDirection, mixFactor);
    }
		...
		for(int y = -range; y <= range; y++)
		{
			for(int z = -range; z <= range; z++)
			{
				if (!(x == 0 && y == 0 && z == 0)) // NOT CURRENT CELL
				{
					...
					{
						ivec3 coord = ivec3((b.position + direction));
						vec4 neighborVelocity = texelFetch(sTD3DInputs[INOUT_DRAW_SPEED_HEALTH_BUFFER], coord, 0);
						float neighborEspece = texelFetch(sTD3DInputs[INOUT_COLOR_AGENTS_BUFFER], coord, 0).r;
			...
			for(int z = -range; z <= range; z++)
			{
				if (!(x == 0 && y == 0 && z == 0)) // NOT CURRENT CELL
				{
					...
					if(dot(normx(direction), mydirection) > angle) // sensing angle
					{
						...
						vec3 neighborVelocity = texelFetch(sTD3DInputs[INOUT_DRAW_SPEED_HEALTH_BUFFER], ivec3(coord), 0).rgb;
						float neighborEspece = texelFetch(sTD3DInputs[INOUT_COLOR_AGENTS_BUFFER], ivec3(coord), 0).r;
		...
		for(int y = -range; y <= range; y++)
		{
			for(int z = -range; z <= range; z++)
			{
				if (!(x == 0 && y == 0 && z == 0)) // NOT CURRENT CELL
				{
					...
					vec3 targetSpeciesAndColor = texelFetch(sTD3DInputs[INOUT_COLOR_AGENTS_BUFFER], ivec3(round(coord.xy)), 0).rba;
					float targetHealth = targetSpeciesAndColor.z;
                    float targetSpecies = targetSpeciesAndColor.x;
                    float targetColor = targetSpeciesAndColor.y;
...
void Write (Agent a, Food foods[7], ivec3 coord, int closest)
{
	vec4 outDrawMassCoord = vec4(0);
	vec4 outDrawSpeedHealth = vec4(0);

    // Check if agent's position is within the boundaries
    bool withinBoundaries = a.position.x >= 0 && a.position.x <= uRes.x && a.position.y >= 0 && a.position.y <= uRes.y && a.position.z >= 0 && a.position.z <= uRes.z;
    ...
    if (AliveAgent(a)) 
    {
		...
		imageStore(mTDComputeOutputs[INOUT_COLOR_AGENTS_BUFFER], ivec3(round(a.position)), TDOutputSwizzle(vec4(a.color)));
		outDrawMassCoord = vec4(a.mass, a.myCoord, a.color.r);
		outDrawSpeedHealth = vec4(a.speed, a.health);
		imageStore(mTDComputeOutputs[INOUT_DRAW_MASS_COORD_BUFFER], ivec3(round(a.position)), TDOutputSwizzle(outDrawMassCoord));
		imageStore(mTDComputeOutputs[INOUT_DRAW_SPEED_HEALTH_BUFFER], ivec3(round(a.position)), TDOutputSwizzle(outDrawSpeedHealth));
		imageStore(mTDComputeOutputs[INOUT_MEMORYFOOD_COLOR_B_BUFFER], coord, TDOutputSwizzle(vec4(a.foodMemory, a.color.b)));
		imageStore(mTDComputeOutputs[OUT_ORIENTATIONS_BUFFER], coord, TDOutputSwizzle(vec4(a.color.rg, a.elevation, a.orientation)));
    }

	else
	{
		imageStore(mTDComputeOutputs[INOUT_DRAW_SPEED_HEALTH_BUFFER], ivec3(round(a.position)), TDOutputSwizzle(vec4(0)));
		imageStore(mTDComputeOutputs[INOUT_DRAW_MASS_COORD_BUFFER], ivec3(round(a.position)), TDOutputSwizzle(vec4(0)));
		imageStore(mTDComputeOutputs[INOUT_COLOR_AGENTS_BUFFER], ivec3(round(a.position)), TDOutputSwizzle(vec4(0)));
		...
		imageStore(mTDComputeOutputs[OUT_TRAILS_BUFFER], ivec3(round(a.position)), TDOutputSwizzle(vec4(0)));
    ...
    bool foodWithinBoundaries = coord.x < uCounts.y && coord.y == 0 && coord.z < uCounts.y;

    if (foodWithinBoundaries) 
    {
        if(coord.x < uCounts.y && coord.y == 0 && coord.z < uCounts.y)
        {
            vec4 foodColor = vec4(foods[coord.x].foodColor.rgb, 1.0);
            imageStore(mTDComputeOutputs[INOUT_DRAW_FOOD_BUFFER], ivec3(round(foods[coord.x].position)), TDOutputSwizzle(foodColor));
            imageStore(mTDComputeOutputs[INOUT_DRAW_SPEED_HEALTH_BUFFER], ivec3(round(foods[coord.x].position)), TDOutputSwizzle(foodColor));
            imageStore(mTDComputeOutputs[INOUT_DRAW_MASS_COORD_BUFFER], ivec3(round(foods[coord.x].position)), TDOutputSwizzle(foodColor));
        }
    }

}

void WriteDiffuseTexture(Agent a, ivec3 posOnBuffer)
{
    float uTrailReinforcementFactor = 0.01;

    // color of present voxel
    vec4 oc = texelFetch(sTD3DInputs[INOUT_COLOR_AGENTS_BUFFER], posOnBuffer, 0);
    vec4 colorPresentVoxel = texelFetch(sTD3DInputs[IN_TRAILS_BUFFER], posOnBuffer, 0);

    // Check if the color is close to the color of species 2 (r = 0.7, g = 0.0, b = 1.0)
    bool isSpecies2 = abs(colorPresentVoxel.r - 0.7) < 0.05 && abs(colorPresentVoxel.g - 0.0) < 0.05 && abs(colorPresentVoxel.b - 1.0) < 0.05;

    // Decay the color by a certain factor if it's not species 2
    if (!isSpecies2 && colorPresentVoxel.a > 0.01) 
    {
        colorPresentVoxel *= uDecayFactorAnts;
        colorPresentVoxel = clamp(colorPresentVoxel, 0, 1);
    }

    float avg = 0;

    // look at surrounding 27 cubes
    for (int x = -1; x <= 1; x++)
    {
        for (int y = -1; y <= 1; y++)
        {
            for (int z = -1; z <= 1; z++)
            {
                // surrounding cube coordinate
                ivec3 coord = (posOnBuffer + ivec3(x, y, z));

                // neighbor species
                float especeNeighbor = texelFetch(sTD3DInputs[INOUT_COLOR_AGENTS_BUFFER], coord, 0).r;

                // average it
                if (especeNeighbor == 0.7)
                {
                    avg += texelFetch(sTD3DInputs[IN_TRAILS_BUFFER], coord, 0).r;
                }
            }
        }
    }
    ...
    // update
    imageStore(mTDComputeOutputs[OUT_TRAILS_BUFFER], posOnBuffer, TDOutputSwizzle(colorPresentVoxel + oc));
}

ivec3 findEmptyCell(ivec3 coord) 
{
    ...
    while(attempt < maxAttempts)
    {
        ...
        int randomZ = int(randomVec4Range(coord, uTime * 53.42, 0.0, float(uRes.z)).z);
        
        potentialCell = ivec3(randomX, randomY, randomZ);
    ...
    return ivec3(-99);  // return a non-valid coordinate if no empty cell found after maxAttempts
}
...
// 	ivec3 mateCoord = ivec3(-99); // Initialize mateCoord
...
// 			if (agent.health < 4 && length(foodTarget) > 0) 
// 			{
// 				// Seek food target
// 				SeekTarget(agent, foodTarget, uSteerConfigs.z, uSeekSettings.z);

// 				if(distance(agent.position, foodTarget) < 10)
// 				{
...
// 				if(uDNA == 1)
// 				{
// 					DNA0scaled = Map(agent.DNA.r, 0, 1, 0.1, 5);
// 					DNA1scaled = Map(agent.DNA.g, 0, 1, 0.0, 1.0);
// 					DNA2scaled = Map(agent.DNA.b, 0, 1, 0.5, 10);
// 				}

// 				vec3 antsTurns = AntPhysarumBehavior(agent, DNA1scaled, DNA0scaled);
// 				vec3 avoidPredatorForce = AvoidPredator(agent, 0.4) * DNA2scaled;

// 				ApplyForce(agent, antsTurns + avoidPredatorForce);				
// 			}
...
// 				if(uDNA == 1)
// 				{
// 					DNA0scaled = Map(agent.DNA.r, 0, 1, 0.5, 10);
// 					DNA1scaled = Map(agent.DNA.g, 0, 1, 1.0, 10);
// 					DNA2scaled = Map(agent.DNA.b, 0, 1, 1.0, 20);
// 				}

				
// 				vec3 chasingForce = ChasePrey(agent, 1.0) * DNA0scaled;
// 				vec3 avoidPredatorForce = AvoidPredator(agent, 0.7) * DNA1scaled;
// 				ApplyForce(agent, chasingForce + avoidPredatorForce);
...
// 						{
// 							ivec3 neighborPos = ivec3(agent.position.x + x, agent.position.y + y); 
// 							neighborPos = ivec3(round(neighborPos));
// 							vec4 neighborInfo = texelFetch(sTD3DInputs[INOUT_DRAW_MASS_COORD_BUFFER], ivec3(neighborPos), 0);
// 							ivec3 neighborCoord = ivec3(neighborInfo.gb);
// 							float neighborColor = neighborInfo.a;

// 							if(neighborColor > 0) //reproduction intra species
// 							{
// 								float neighborHealth = texelFetch(sTD3DInputs[INOUT_DIRECTION_AND_HEALTH_BUFFER], ivec3(neighborCoord), 0).a;
...
// 									{
// 										ivec3 emptyCell = findEmptyCell(coord); // TODO thing is in parallel so all agents will find an empty spot at the same time, fix that

// 										Agent child;

// 										if (emptyCell != ivec3(-99))
// 										{
// 											child.myCoord = emptyCell;
// 											child = crossoverAndMutate(child.myCoord, agent, mateCoord);
// 											UpdateAgent(child);
// 											WriteDiffuseTexture(child, child.myCoord);
...
// 									{
// 										ivec3 emptyCell = findEmptyCell(coord);
// 										Agent child;
