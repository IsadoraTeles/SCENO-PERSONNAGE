...
vec3 AntPhysarumBehavior(inout Agent a, float forceFactor)
{
	...
	int rangeNeighbors = int(Map(b.DNA.r, 0, 1, 2, 6));
	int senseAngleAnts = int(Map(b.DNA.g, 0, 1, 10, 90));
	int rotationAngleAnts = int(Map(b.DNA.b, 0, 1, 40, 100));
...
vec3 Align (Agent a)
{
	...
	int rangeNeighbors = int(Map(b.DNA.r, 0, 1, 1, 5));
	float angleDNA = Map(b.DNA.g, 0, 1, 0.2, 0.7);
...
vec3 AvoidPredator(inout Agent agent, float avoiding) 
{
	...
	float maxSteerForce = Map(bagent.DNA.r + bagent.DNA.b, 0, 2, 0.5, 4);
