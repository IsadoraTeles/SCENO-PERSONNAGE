			...
			for(int z = -uRangeNeighbors.x; z <= uRangeNeighbors.x; z++)
			{
				if (!(x == 0 && y == 0 && z == 0)) // NOT CURRENT CELL
				{
                    ...
                    if(targetHealth > 0)
                    {
						...
						/*
                        if(a.color.b == 0.4 && targetColor == 0.7 )
						{
							float strenght = -uG * (a.mass * targetMassCoord.r) / distSq;
						...
						else if(a.color.b == 0.9 && targetColor == 0.7 )
						{
							float strenght = uG * (a.mass * targetMassCoord.r) / distSq;
						...
						else if(targetColor == 0.7 && targetColor == 0.9 )
						{
							float strenght = -uG * 1.5 * (a.mass * targetMassCoord.r) / distSq;
						...
						else if(targetColor == 0.7 && targetColor == 0.4 )
						{
							float strenght = uG * 1.5 * (a.mass * targetMassCoord.r) / distSq;
						...
						else if(targetColor == 0.4 && targetColor == 0.9 )
						{
							float strenght = uG * 0.358 * (a.mass * targetMassCoord.r) / distSq;
						...
						else if(targetColor == 0.9 && targetColor == 0.4 )
						{
							float strenght = -uG * 0.358 * (a.mass * targetMassCoord.r) / distSq;
						...
						*/
						...
						if(dist > uMinMaxDistandCroudMin.x && dist < uMinMaxDistandCroudMin.y)
						{
							...
							else if(targetSpecies == 0.4 && targetColor == a.color.b ) // attraction 1
							{
								float strenght = uG * 1.5 * (a.mass * targetMass) / distSq;
							...
							else if(targetSpecies == 0.4 && targetColor < 0.5 && a.color.b >= 0.5) // repulsion 0.5
							{
								float strenght = -uG * 0.6 * (a.mass * targetMass) / distSq;
							...
							else if(targetSpecies == 0.4 && targetColor < 0.8 && a.color.b >= 0.8) // repulsion 1.5
							{
								float strenght = uG * (a.mass * targetMass) / distSq;
...
void main()
{
	...
	if (ActiveAgent()) // if agent exists
	{
		if (AliveAgent(agent))
		{
			...
			if (closest > -1 && agent.health < 20)
			{
				SeekTarget(agent, foods[closest].position, uSeekSettings.z, uSeekSettings.x);
			}
