...
uniform vec2 uWindDirection; // direction control (e.g., vec2(1.0, 0.0) for right wind)
uniform float uWindStrength; // strength control
...
struct Agent
{
    ...
    float minSpeed;
    vec3 acceleration;
    vec3 force; // New field for storing the force
};
...
vec3 curlNoise(vec3 p)
{
    ...
    vec3 p_x0 = BitangentNoise3D(p - dx);
    vec3 p_x1 = BitangentNoise3D(p + dx);
    vec3 p_y0 = BitangentNoise3D(p - dy);
    vec3 p_y1 = BitangentNoise3D(p + dy);
    vec3 p_z0 = BitangentNoise3D(p - dz);
    vec3 p_z1 = BitangentNoise3D(p + dz);
...
Agent ReadAgent(ivec2 coord)
{
    ...
    a.maxSpeed = Map(a.DNA.b, 0, 1, 0.3 * uSteerConfigs.y, uSteerConfigs.y);
    a.minSpeed = a.DNA.b * uSteerConfigs.y * 0.3;
...
Agent ResetAgents(ivec2 posOnTexture) 
{
    ...
    vec3 posRandom = vec3(
        randomRange(posOnTexture + sin(uTime * .49), 0.0, uRes.x),
        randomRange(posOnTexture + cos(uTime * .899), 0.0, uRes.y),
        randomRange(posOnTexture * sin(uTime * .0979), 0.0, uRes.z)
    );

    vec3 velRandom = vec3(
        randomRange(posOnTexture + sin(uTime * .49), -1.0, 1.0), 
        randomRange(posOnTexture + cos(uTime * .899), -1.0, 1.0), 
        randomRange(posOnTexture * sin(uTime * .0979), -1.0, 1.0)
    );
    ...
    a.speed = velRandom;
    a.direction = normalize(velRandom) * 2.0 - 1.0;
...
void ApplyForce(inout Agent a, vec3 force)
{
    ...
    a.force += force; // Accumulate the force
}

void ApplyWind(inout Agent a) 
{
    // Create wind vector from direction and strength
    vec3 wind = vec3(uWindDirection, 0.0);
    
    // Apply wind to the agent's force
    ApplyForce(a, wind * uWindStrength); // You can adjust this multiplier as needed
}
...
void SeekTarget(inout Agent a, vec3 target, float mag, float rad)
{
    ...
    float newMaxSpeed = a.maxSpeed;

    if(dist < rad/2) // very close to the target
    {
        desired *= Map(dist, 0, rad/2, 0, newMaxSpeed);
        vec3 steer = desired - a.speed;
        steer = clamp(steer, vec3(-uSteerConfigs.z), vec3(uSteerConfigs.z)); // using uSteerConfigs.z for max force
    ...
    else if(dist < rad) // close to the target
    {
        ...
        vec3 steer = desired - a.speed;
        steer = clamp(steer, vec3(-uSteerConfigs.z), vec3(uSteerConfigs.z));
        ApplyForce(a, steer * mag);
    }
...
{
    vec2 b = p + v;
    return ((b.x - p.x) * (op.y - p.y) - (b.y - p.y) * (op.x - op.y)) > 0.0;
}
...
vec3 OrientationForcePPSAgent(inout Agent agent, float alpha, float beta) 
{
    Agent b = agent;
    ...
    b.orientation += delta_phi;
    b.orientation = mod(b.orientation, TWOPI);

    vec3 direction = vec3(cos(b.orientation), sin(b.orientation), 0.0);

    float h = 0.5;
    float s = 1.0;
    float v = clamp(n / 10.0, 0.0, 1.0);
...
vec3 AntPhysarumBehavior(inout Agent a, float forceFactor)
{
    ...
    int rangeNeighbors = int(Map(b.DNA.r, 0, 1, uRangeNeighbors.z, uRangeNeighbors.z + 2));
    int senseAngleAnts = int(Map(b.DNA.g, 0, 1, uConfigsAnts.x, uConfigsAnts.x + 20));
    int rotationAngleAnts = int(Map(b.DNA.b, 0, 1, uAntRadians2, uAntRadians2 + 30));

    float SO = rangeNeighbors; // sensor offset
    float SA = radians(senseAngleAnts); 
    float RA = radians(rotationAngleAnts); // rotation angle that we rotate by for turns
...
vec3 Align (Agent a)
{
    ...
    int rangeNeighbors = int(Map(b.DNA.r, 0, 1, 1, uRangeNeighbors.y));
    float angleDNA = Map(b.DNA.g, 0, 1, 0.1, uSeekSettings.y + 0.2);

    float angle = angleDNA;
    vec3 avg = vec3(0);

    int range = rangeNeighbors; // sensing distance 
    int total = 0;
    vec3 steer = vec3(0);
...
vec3 AvoidPredator(inout Agent agent, float avoiding) 
{
    ...
    float maxSteerForce = uSteerConfigs.z; // using uSteerConfigs.z for max force
...
// Update Agent function with noise application
void UpdateAgent(inout Agent agent)
{
    if (AliveAgent(agent)) 
    {
        // Apply wind to the agent's acceleration
        ...
        speed = min(speed, uSteerConfigs.y);
        speed = clamp(speed, agent.minSpeed, agent.maxSpeed);
        agent.speed = direction * speed;
        agent.direction = direction;
        agent.position += agent.speed;  // Update position with speed and delta time

        agent.force = agent.acceleration * agent.mass; // Store the force before resetting the acceleration
...
void Write(Agent a, Food foods[13], ivec2 coord, int closest)
{
    ...
    if (AliveAgent(a)) 
    {
        ...
        float maxForce = 10.0; // Define a maximum possible force to normalize against
        float actualForce = length(a.force);

        // Avoid division by zero and handle edge cases
        float normalizedForce = maxForce > 0.0 ? actualForce / maxForce : 0.0;
        if (actualForce > maxForce) normalizedForce = 1.0; // Clamp normalizedForce to [0, 1]
        ...
        if (a.espece == 3)
        {
            ...
            float l = rl.y;
            float n = r + l;
            float normalizedN = clamp(n / 10.0, 0.5, 1.0);

            imageStore(mTDComputeOutputs[OUT_ORIENTATIONS_BUFFER], coord, TDOutputSwizzle(vec4(1/normalizedN, a.DNA.g, normalizedForce, a.orientation)));
        }
        else
        {
            // For ants and boids (especes 1 and 2)
            imageStore(mTDComputeOutputs[OUT_ORIENTATIONS_BUFFER], coord, TDOutputSwizzle(vec4(a.DNA.r, a.DNA.g, a.DNA.b, normalizedForce)));
        }
    ...
    if (ActiveAgent(coord)) // if agent exists
    {
        ...
        if (AliveAgent(agent))
        {
            ...
            if (agent.health < 5)
            {
                ...
                {
                    float maxSteerForce = uSteerConfigs.z; // using uSteerConfigs.z for max force
                    float seekfood = Map(agent.DNA.r + agent.DNA.g, 0, 2, 1, uSeekSettings.z);
            ...
            else if (agent.health < 10)
            {
                ...
                if(length(foodTarget) > 0)
                {
                    ...
                    SeekTarget(agent, foodTarget, maxSteerForce, seekfood);
            ...
            if(agent.espece == 1 ) // BOIDS
            {
                if(uDNA == 1)
                {
                    DNA0scaled = Map(agent.DNA.r, 0, 1, 1, 5);
                    DNA1scaled = Map(agent.DNA.g, 0, 1, 1, 5);
                    DNA2scaled = Map(agent.DNA.b, 0, 1, 1, 5);
                }
                ...
                vec3 avoidPredatorForce1 = AvoidPredator(agent, 0.4) * DNA0scaled * 2;
            ...
            else if(agent.espece == 2) // PHYSARUM
            {
                if(uDNA == 1)
                {
                    ...
                    DNA1scaled = Map(agent.DNA.g, 0, 1, 0.0, 3);
                    DNA2scaled = Map(agent.DNA.b, 0, 1, 0.5, 4);
                }
            ...
            else if (agent.espece == 3) // PPS
            {
                if(uDNA == 1)
                {
                    DNA0scaled = Map(agent.DNA.r, 0, 1, 1, 4);
                    DNA1scaled = Map(agent.DNA.g, 0, 1, -15, 15);
                    DNA2scaled = Map(agent.DNA.b, 0, 1, -3, 3);
                    DNA3scaled = Map(agent.DNA.g, 0, 1, 2, 5);
                }

                vec3 avoidPredatorForce1 = AvoidPredator(agent, 0.7) * DNA0scaled;
                vec3 avoidPredatorForce3 = AvoidPredator(agent, 1.0) * DNA2scaled;
                vec3 orientatioForce = OrientationForcePPSAgent(agent, radians(uAlphaPPS + DNA1scaled), radians(uBetaPPS + DNA1scaled)) * DNA3scaled ;
                
                ApplyForce(agent, avoidPredatorForce3 + avoidPredatorForce1 + orientatioForce);
            ...
            if(agent.health >= 4) // REPRODUCTION DYNAMICS
            {
                ...
                int rangeNeighbors = int(Map(agent.DNA.r, 0, 1, 1, 3));
