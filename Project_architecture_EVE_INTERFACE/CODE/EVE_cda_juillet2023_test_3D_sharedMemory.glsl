...
// BINNING BUFFERS
// Assuming agentsTexture (INOUT_POSITION_AND_MASS_BUFFER) and hashTableTexture (HASH_TABLE_TEXTURE) are samplers for your 2D textures, 
// and agentsCountBuffer and cellSize are uniform variables
#define HASH_TABLE 13 // unsigned integers ?
const vec3 scale = vec3(73.0, 79.0, 83.0); // Random prime numbers

#define CELL_SIZE 32
#define MAX_AGENTS_PER_CELL 128
#define HASHTABLESIZE 512
#define TOTAL_CELLS 256

// ATOMIC COUNTERS
layout(binding = 0, offset = 0) uniform atomic_uint binCountsAtomic;

layout (local_size_x = 8, local_size_y = 8) in;

// TODO : FLOOR floats into int indexes
...
/*
Setup: At the start, you need to define the spatial hash table. This is usually a 2D texture where each pixel can store the index of an agent. The texture dimensions depend on the size of the world and the desired spatial resolution (cell size). For simplicity, the world is often assumed to be a square and the hash table a square texture. The maximum number of agents per cell must also be defined, which will determine the depth of the texture (or the use of multiple textures).
*/

void initializeHashTable(Grid grid) 
{
    for (int i = 0; i < GRID_SIZE; i++) 
	{
		for(int j = 0; j < GRID_SIZE; j++)
		{
			imageStore(mTDComputeOutputs[HASH_TABLE], ivec2(i, j), TDOutputSwizzle(vec4(0.0)));
		}
    }
}

/*
Position to Hash: Each agent's position is mapped to a hash cell. The hash function typically computes the floor division of each coordinate by the cell size. This essentially "snaps" each agent to the grid defined by the hash table. For example, if the cell size is 10, agents at positions (15, 15, 15) and (16, 16, 16) would be mapped to the same hash cell (1, 1, 1). The hash function should produce a unique hash for each cell.

The atomic operations are used in the example to handle writing the agent's data into the hash table and to handle keeping count of the number of agents written to each cell of the hash table.
*/

ivec2 hash(inout Agent agent) 
{
    // Calculate the index in each dimension
    ivec3 index3D = ivec3(agent.position * scale);
    
    // Use bitwise operations to combine the 3D index into a 2D one
    int hashValue = int(dot(index3D.xyz, vec3(1, 31, 997))); // 31 and 997 are prime numbers

    // Convert scalar value to a 2D coordinate
    ivec2 hashIndex;
    hashIndex.x = hashValue % hashTableSize.x;
    hashIndex.y = (hashValue / hashTableSize.x) % hashTableSize.y;

	agent.hashIndex = hashIndex;

    return hashIndex;
}

/*
Writing to Hash Table: For each agent, we compute its hash. We then try to write the agent's data (such as its 2d coordinate or position) to the corresponding location in the hash table. If there is a limit on the number of agents per cell, we also need to keep track of how many agents have been written to each cell. This can be done using atomic counters.
*/

void addToHashTable(Agent agent) 
{
    ivec3 gridPos = ivec3(agentPos.x / CELL_SIZE, agentPos.y / CELL_SIZE, agentPos.z / CELL_SIZE);
    int hashIndex = calcSpatialHash(gridPos);

    // Add agent to hash table using atomic operation
    uint offset = atomicAdd(counts[hashIndex], 1);
    if (offset < MAX_AGENTS_PER_CELL) 
	{
        imageStore(HASH_TABLE, ivec2(hashIndex * MAX_AGENTS_PER_CELL + offset, 0), vec4(agent.myCoord));
    }
}

/*
Reading from Hash Table: To find an agent's neighbors, we compute the hash for the agent's position as before. We then check not only the corresponding cell in the hash table, but also its adjacent cells. The data found in these cells are considered as the agent's neighbors.
*/



/*
Update Loop: After all agents have been processed (their behaviors updated based on their neighbors), we need to reset the hash table and the counters for the next frame. This can be done by simply setting all values in the hash table to a null value and all counters to zero.
*/
...
{
	vec2 pos = mod(coord + uRes, uRes);
	int index = (int(pos.x) + (int(pos.y) * int(uRes.x)));
	return index;
}
...
Agent ResetAgents(ivec2 posOnTexture)
{
	...
	vec3 p = center + 100 * vec3(cos(angle), sin(angle), 0.0);
...
vec3 rotateAroundUp(vec3 v, float angle) // 3d test rotation
{
    ...
    return vec3(
        oc * up.x * up.x * v.x + oc * up.x * up.y * v.y - up.z * s * v.y + oc * up.x * up.z * v.z + up.y * s * v.z,
        oc * up.x * up.y * v.x + s * up.z * v.x + oc * up.y * up.y * v.y + oc * up.y * up.z * v.z - up.x * s * v.z,
        oc * up.x * up.z * v.x - up.y * s * v.x + oc * up.y * up.z * v.y + s * up.x * v.y + oc * up.z * up.z * v.z
    );
}
    ...
    if(isSpecies2)
    {
        ...
        for (int x = -1; x <= 1; x++)
        {
            for (int y = -1; y <= 1; y++)
            {
				...
				vec2 coord = mod(posOnBuffer + vec2(x, y) + uRes.x, uRes.x);
...
ivec2 findEmptyCell(ivec2 coord) 
{
    ...
    while(attempt < maxAttempts)
    {
        ...
        int randomY = int(randomVec4Range(coord, uTime * 25.21, 0.0, float(uRes.x)).y);
...
void main()
{
	...
	Grid newGrid ;
	newGrid.dimensions = vec2(uRes.x, uRes.y);
	newGrid.sizeCells = 32;
	newGrid.cols = ceil(uRes.x / sizeCells);
	newGrid.rows = ceil(uRes.y / sizeCells);
	...
	if (ActiveAgent(coord)) // if agent exists
	{
		...
		if (AliveAgent(agent))
		{
			...
			else if(agent.espece == 2) // PHYSARUM
			{
				...
				vec3 avoidPredatorForce = AvoidPredator(agent, 0.4);
				vec3 avoidboidsForce = AvoidBoids(agent);

				ApplyForce(agent, turnForce + avoidPredatorForce * DNA1scaled + avoidboidsForce * DNA2scaled);				
			}
			...
			else if (agent.espece == 3) // PP
			{
				...
				vec3 chasingForce = ChasePrey(agent, 0.4) * DNA0scaled;
				vec3 avoidPredatorForce = AvoidPredator(agent, 1.0) * DNA1scaled;
				vec3 avoidPredatorForce2 = AvoidPredator(agent, 0.7) * DNA1scaled;
				ApplyForce(agent, chasingForce + avoidPredatorForce + avoidPredatorForce2);
