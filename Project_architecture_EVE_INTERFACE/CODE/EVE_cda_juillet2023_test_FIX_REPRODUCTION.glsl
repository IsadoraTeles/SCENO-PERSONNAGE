...
// ivec2 findEmptyCell(ivec2 center, int range) // find empty cell and color it with a number
// {
// 	Food foods[7];
...
//         for (int y = -1; y <= 1; y++) 
// 		{
//             ivec2 neighborCoord = ivec2(center.x + x, center.y + y);

//             if (ActiveAgent(neighborCoord))
// 			{
// 				Agent neighborAgent = ReadAgent(neighborCoord);

// 				if(neighborAgent.health <= 0.0)
// 				{
...
//         }
//     }
//     return ivec2(-99); // return a non-valid coordinate if no empty cell found
// }
	...
	if (ActiveAgent(coord)) // if agent exists
	{
		...
		if (AliveAgent(agent))
		{
			...
			if(agent.espece == 1 ) // BOIDS
			{
				...
				vec3 avoidPredatorForce1 = AvoidPredator(agent, 0.4) * DNA0scaled * 30;
			...
			else if (agent.espece == 3) // PP
			{
				if(uDNA == 1)
				{
					...
					DNA2scaled = Map(agent.DNA.b, 0, 1, 0.2, 10);
				}

				
				vec3 chasingForce = ChasePrey(agent, 1.0) * DNA0scaled * 20;
				vec3 avoidPredatorForce = AvoidPredator(agent, 0.7) * DNA1scaled * 100;
						...
						if (!(x == 0 && y == 0)) // NOT CURRENT CELL
						{
							...
							if(neighborColor > 0) //reproduction intra species
							{
								...
								if(neighborHealth >= 2)
								{
									...
									// SELF REPRODUCTION
									else if (born < uReproductionChance && validMateFound == false && agent.health >= 3)
									{
