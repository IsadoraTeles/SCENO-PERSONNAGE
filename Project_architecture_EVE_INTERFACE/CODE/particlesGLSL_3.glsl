...
uniform int uRange;
uniform vec2 uMapXMaxMin;
uniform vec2 uMapYMaxMin;
uniform int uRes;
uniform float uG;
uniform vec3 uMinMaxDistandCroudMin;
...
#define IN_DRAW_AGENTS_BUFFER 6
#define IN_STATE_BUFFER 7
	...
	for (int x = -uRange; x <= uRange; x++)
	{
		for(int y = -uRange; y <= uRange; y++)
		{
			for(int z = -uRange; z <= uRange; z++)
			{
				...
				{
                    float posposX = map(b.position.x, uMapXMaxMin.y, uMapXMaxMin.x, 0, uRes);
                    float posposY = map(b.position.y, uMapYMaxMin.y, uMapYMaxMin.x, 0, uRes);
                    vec2 pospos = round(vec2(posposX, posposY));

                    vec4 target = texelFetch(sTD2DInputs[IN_DRAW_AGENTS_BUFFER], ivec2(pospos.x + x, pospos.y + y), 0);
                    ...
                    if(target.r > 0.001)
                    {
						...
						if(targetColor < 0.4 && dist > uMinMaxDistandCroudMin.x + 0.5 && dist < uMinMaxDistandCroudMin.y)
						{
							...
							vec3 steer = clamp(force, vec3(-uAbsMaxSteerForce), vec3(uAbsMaxSteerForce));
		...
		for(int y = -uRange; y <= uRange; y++)
		{
			for(int z = -uRange; z <= uRange; z++)
			{
				if (!(x == 0 && y == 0 && z == 0)) // NOT CURRENT CELL
				{
                    ...
                    if(target.r == 0.0)
                    {
						vec2 targetPos = vec2(pospos.x + x, pospos.y + y);
                        vec2 direction = pospos - targetPos;// direction
...
void Update(inout Agent agent, vec4 initial) // inout is like a pointer
{
	...
	agent.speed = clampVector(agent.speed, uMaxSpeed);
...
void Write (Agent a, ivec2 coord)
{
    ...
    vec4 outDraw = vec4(a.mass, a.myCoord, a.color.r);
    float posposX = map(a.position.x, uMapXMaxMin.y, uMapXMaxMin.x, 0, uRes);
    float posposY = map(a.position.y, uMapYMaxMin.y, uMapYMaxMin.x, 0, uRes);
    vec2 pospos = vec2(posposX, posposY);
    imageStore(mTDComputeOutputs[IN_DRAW_AGENTS_BUFFER], ivec2(round(pospos)), TDOutputSwizzle(outDraw));
...
void main()
{
	...
	//vec2 collisions  = texelFetch(sTD2DInputs[IN_COLLISIONS_BUFFER], ivec2(gl_GlobalInvocationID.xy), 0).rg;
    //agent.acceleration.rg *= collisions;
	...
	GravitationalAttraction(agent, uAttractionForce);
	Attractors(agent);
